 ...
 * @see #set(String)
 */
public class NullTerminatedUTF16String extends NativeInt8Array {
     ...
     * @throws IndexOutOfStructRange if given {@code value} (and two 0-bytes) does not fit into this structure
     * @param value {@link String} value
     */
    public void set(@NotNull String value) throws IndexOutOfStructRange {
        byte[] bytes = value.getBytes(StandardCharsets.UTF_16);

        if(bytes.length + 2 > length)
            throw new IndexOutOfStructRange(bytes.length + 1, this);

        set(bytes);
        setInt8(bytes.length, (byte) 0);
        setInt8(bytes.length + 1, (byte) 0);
    }
